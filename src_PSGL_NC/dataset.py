...
from utils import precompute_dist_data, get_link_mask, duplicate_edges, deduplicate_edges, get_dist_max,split_for_NodeClassification
...
def get_tg_dataset2(args, dataset_name, use_cache=True, remove_feature=False):       # reads data, puts in nx object
    edge_labels = [None]
        ...
        for i, data in enumerate(dataset):
            ...
            if args.task == 'link_pair':
                if dataset_name in ['Cora','CiteSeer', 'PubMed']:
                    ...
                    node_ids=[]
                    for nodeid in range(n1):
                        node_ids.append(nodeid)


                    data.num_classes=int(max(data.y)) + 1
                    Node_Classification_Train,Node_Classification_Val,Node_Classification_Test=split_for_NodeClassification(node_ids,args.remove_link_ratio)
                    data.Node_Classification_Train = Node_Classification_Train
                    data.Node_Classification_Val   = Node_Classification_Val
                    data.Node_Classification_Test  = Node_Classification_Test

                    # print(use_cache)
                    # print(os.path.isfile(f2_name))
                    # print(args.task ,args.task=='link')
                    # print(os.path.isfile(f1_name))
                    # print(args.task!='link')
            ...
            if args.task=='link':
                ...
                dists_removed = precompute_dist_data(data.edge_index.numpy(), data.num_nodes, approximate=args.approximate)
...
def get_tg_dataset(args, dataset_name, use_cache=True, remove_feature=False):       # reads data, puts in nx object
    ...
    if dataset_name in ['Cora', 'CiteSeer', 'PubMed']:
        ...
        dataset = tg.datasets.CitationFull(root='datasets/', name=dataset_name)
    elif dataset_name in ['PROTEINS']:        
        dataset = TUDataset(os.path.join('datasets'),name='PROTEINS')  
    elif dataset_name in ['Photo']:
        dataset = tg.datasets.Amazon(root='datasets/', name='Photo')  

    elif dataset_name in ['acm', 'BlogCatalog', 'flickr', 'uai']:
        dataset = []
        feature = './datasets/data_file/' + dataset_name + '/' + dataset_name + '.feature'
        label = './datasets/data_file/' + dataset_name + '/' + dataset_name + '.label'
        edge = './datasets/data_file/' + dataset_name + '/' + dataset_name + '.edge'

        x = np.loadtxt(feature, dtype = float)
        y = np.loadtxt(label, dtype = int)
        edge_index = np.genfromtxt(edge, dtype=np.long).T
    ...
    f5_name = 'datasets/cache/' + args.task + dataset_name + str(args.select_anchors)+ '_links_test.dat'
    f0_name = 'datasets/cache/' + args.task + dataset_name + str(args.select_anchors)+ '_links.dat'
    f8_name = 'datasets/cache/' + args.task + dataset_name + str(args.select_anchors)+ '_nodes_train.dat'
    f9_name = 'datasets/cache/' + args.task + dataset_name + str(args.select_anchors)+ '_nodes_val.dat'
    f10_name = 'datasets/cache/' + args.task + dataset_name + str(args.select_anchors)+ '_nodes_test.dat'
    ...
    if use_cache and ((os.path.isfile(f2_name) and args.task=='link') or (os.path.isfile(f1_name) and args.task!='link')):
        with open(f3_name, 'rb') as f3, \
            ...
            open(f5_name, 'rb') as f5, \
            open(f7_name, 'rb') as f7, \
            open(f8_name, 'rb') as f8, \
            open(f9_name, 'rb') as f9, \
            open(f10_name, 'rb') as f10, \
            open(f0_name, 'rb') as f0:
            links_train_list = pickle.load(f3)
            links_val_list = pickle.load(f4)
            links_test_list = pickle.load(f5)
            ...
            nodes_train_list = pickle.load(f8)
            nodes_val_list = pickle.load(f9)
            nodes_test_list = pickle.load(f10)
        ...
        for i, data in enumerate(dataset):
            ...
            data.Node_Classification_Train = nodes_train_list[i]
            data.Node_Classification_Val = nodes_val_list[i]
            data.Node_Classification_Test = nodes_test_list[i]
        ...
        for i, data in enumerate(dataset):
            ...
            if args.task == 'link_pair':
                if dataset_name in ['Cora','CiteSeer', 'PubMed', 'DBLP', 'PROTEINS', 'Photo', 'acm', 'BlogCatalog', 'flickr', 'uai']:
                    y_class = data.y.cpu()
                    y_class = y_class.numpy()
            ...
            nodes_train_list.append(data.Node_Classification_Train)
            nodes_val_list.append(data.Node_Classification_Val)
            nodes_test_list.append(data.Node_Classification_Test)
        ...
        with open(f1_name, 'wb') as f1, \
            ...
            open(f4_name, 'wb') as f4, \
            open(f5_name, 'wb') as f5, \
            open(f8_name, 'wb') as f8, \
            open(f9_name, 'wb') as f9, \
            open(f10_name, 'wb') as f10, \
            open(f7_name, 'wb') as f7:
            ...
            # pickle.dump(bipartite_list, f6)
            pickle.dump(edge_labels, f7)
            pickle.dump(nodes_train_list, f8)
            pickle.dump(nodes_val_list, f9)
            pickle.dump(nodes_test_list, f10)

        print('Cache saved!')
    return data_list,bipartite_list,edge_labels,data_dists_list


def nx_to_tg_data(graphs, features, orignal_labels=None, edge_labels=None):
    data_list = []
    for i in range(len(graphs)):
        ...
        for m in range(feature.shape[0]):
            ...
            # y[graph_nodes[m]] = orignal_labels[m]
...
def load_graphs(args,dataset_str):
    ...
    if dataset_str == 'grid':
        ...
        for g in graphs:
            ...
            orignal_lbl = np.zeros((n),dtype=int)

            for i, u in enumerate(g.nodes()):
                orignal_lbl[i]=label_all[int(u)][1]
            ...
            orignal_labels.append(orignal_lbl)
    ...
    return graphs, features, orignal_labels, edge_labels, node_labels, idx_train, idx_val, idx_test


def load_tg_dataset(args,name='communities'):
    graphs, features, orignal_labels, edge_labels,_,_,_,_ = load_graphs(args,name)
    return nx_to_tg_data(graphs, features,orignal_labels, edge_labels)
