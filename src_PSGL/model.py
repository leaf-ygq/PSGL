...
class GraphReach_Layer(nn.Module):
    ...
    def forward(self, feature, dists_max, dists_argmax,dists_max2, score):
        if self.dist_trainable:
        ...
        dists_max2 = dists_max2 * score.unsqueeze(0)
        ...
        if args.attention:
            # feature=args.num_walks*feature
...
class PGNN(torch.nn.Module):
    def __init__(self, input_dim, feature_dim, hidden_dim, output_dim,
        ...
        if layer_num>1:
            self.conv_hidden = nn.ModuleList([PGNN_layer(hidden_dim, hidden_dim) for i in range(layer_num)])
...
class P_A_GCN2(torch.nn.Module):
    def __init__(self, input_dim, feature_dim, hidden_dim, output_dim,
                 feature_pre=True, layer_num=2, dropout=True, **kwargs):
        super(P_A_GCN2, self).__init__()
        self.feature_pre = feature_pre
        self.layer_num = layer_num 
        self.dropout = dropout
        ...
        # self.deal_add = nn.Linear(hidden_dim * 3, hidden_dim)
        # self.deal_add2 = nn.Linear(hidden_dim * 2, hidden_dim)
        # # self.deal_add2 = nn.Linear((hidden_dim * 2 + tmp), hidden_dim)
        ...
        self.conv_hidden = nn.ModuleList([GraphReach_Layer(hidden_dim, hidden_dim) for i in range(layer_num)])
        self.conv_out = GraphReach_Layer(hidden_dim, output_dim)

        self.conv_gcn_hidden = nn.ModuleList([tg.nn.GCNConv(hidden_dim, hidden_dim) for i in range(layer_num)])
    ...
    def forward(self, data):
        ...
        if self.feature_pre:
            x = self.linear_pre(x)
        x_position, x_ = self.conv_first(x, data.dists_max, data.dists_argmax, data.dists_max2)
        ...
        add = torch.stack([x_position, x_gcn], dim = 1)
        ...
        for i in range(self.layer_num):
            ...
            x_position, x_ = self.conv_hidden[i](x_position, data.dists_max, data.dists_argmax, data.dists_max2)
        ...
        x_position, x_ = self.conv_out(x_position, data.dists_max, data.dists_argmax, data.dists_max2)
...
# class P_A_GIN(torch.nn.Module):
    def __init__(self, input_dim, feature_dim, hidden_dim, output_dim,
        ...
        self.conv_hidden_nn = nn.ModuleList([nn.Linear(hidden_dim, hidden_dim) for i in range(layer_num)])
        self.conv_gcn_hidden = nn.ModuleList([tg.nn.GINConv(self.conv_hidden_nn[i]) for i in range(layer_num)])
...
# class P_A_GIN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        if self.feature_pre:
            x = self.linear_pre(x)
        x_position, x_ = self.conv_first(x, data.dists_max, data.dists_argmax)
        x_gcn = self.conv_gcn_first(x, edge_index)
        x_gcn = F.relu(x_gcn)
        ...
        for i in range(self.layer_num):
            # _, x_ = self.conv_hidden[i](x_, data.dists_max, data.dists_argmax)
            x_position, x_ = self.conv_hidden[i](x_position, data.dists_max, data.dists_argmax)
            x_gcn = self.conv_gcn_hidden[i](x_gcn, edge_index)
            x_gcn = F.relu(x_gcn)
        ...
        # x_position, x_ = self.conv_out(x_, data.dists_max, data.dists_argmax)
        x_position, x_ = self.conv_out(x_position, data.dists_max, data.dists_argmax)
        x_gcn = self.conv_gcn_out(x_gcn, edge_index)
...
class P_A_GIN2(torch.nn.Module):
    def __init__(self, final_emb_size, input_dim, feature_dim, hidden_dim, output_dim,
                 feature_pre=True, layer_num=2, dropout=True, **kwargs):
        super(P_A_GIN2, self).__init__()
        self.feature_pre = feature_pre
        self.layer_num = layer_num 
        self.dropout = dropout
        ...
        self.lin2 = nn.Linear(final_emb_size, output_dim)
    ...
    def forward(self, data):
        ...
        for i in range(self.layer_num - 2):
            ...
            x_position, x_ = self.conv_hidden[i](x_, data.dists_max, data.dists_argmax)
            x_gcn = self.conv_gcn_hidden[i](x_gcn, edge_index)
            x_gcn = F.relu(x_gcn)
        ...
        # add = F.normalize(x_position, p=2, dim=-1)
...
class GraphReach2(torch.nn.Module):
    def __init__(self, device, final_emb_size, input_dim, feature_dim, hidden_dim, output_dim,
                feature_pre=True, layer_num=2, dropout=True, **kwargs):
        super(GraphReach2, self).__init__()
        self.feature_pre = feature_pre
        self.layer_num = layer_num
        self.dropout = dropout
        if layer_num == 1:
        ...
        self.pool2 = SAGPooling(hidden_dim, ratio=0.5, GNN=GCNConv)
    ...
    def forward(self, data, dists):
        ...
        x = self.Conv1(x, edge_index) #这一步考虑是否删除
        _, _, _, _, perm, score = self.pool(x, edge_index, None, None)   

        # x2 = self.Conv1(x, edge_index)
        # x2, edge_index2, _, _, perm, score = self.pool(x2, edge_index, None, None)
        # x2 = self.Conv2(x2, edge_index2)
        # x2, edge_index2, _, _, perm, score = self.pool2(x2, edge_index2, None, None)
        ...
        # data.dists_max, data.dists_argmax, data.dists_max2 = self.get_our_dist(anchorset_id, dists, self.device)
        data.dists_max, data.dists_argmax = self.get_our_dist(anchorset_id, dists, self.device)

        x_position, x = self.conv_first(x, data.dists_max, data.dists_argmax, score)
        ...
        for i in range(self.layer_num-2):
            # print(x.shape)
            # pdb.set_trace()
            _, x = self.conv_hidden[i](x, data.dists_max, data.dists_argmax, score)
            # x = F.relu(x) # Note: optional!
            if self.dropout:
                x = F.dropout(x, training=self.training)

        x_position, x = self.conv_out(x, data.dists_max, data.dists_argmax, score)
        x_position = F.normalize(x_position, p=2, dim=-1)
        return x_position
...
class P_A_GIN(torch.nn.Module):
    ...
    # def get_our_dist2(self, anchorset_id, dists, device):
...
class P_A_GAT(torch.nn.Module):
    def __init__(self, device, final_emb_size, input_dim, feature_dim, hidden_dim, output_dim,
                 feature_pre=True, layer_num=2, dropout=True, **kwargs):
        super(P_A_GAT, self).__init__()
        self.feature_pre = feature_pre
        self.layer_num = layer_num 
        self.dropout = dropout
        ...
        if feature_pre:
            ...
            # self.conv_first_nn = nn.Linear(feature_dim, hidden_dim)
            # self.conv_gcn_first = tg.nn.GINConv(self.conv_first_nn)
            self.conv_gcn_first = tg.nn.GATConv(feature_dim, hidden_dim)
            ...
            # self.conv_first_nn = nn.Linear(input_dim, hidden_dim)
            # self.conv_gcn_first = tg.nn.GINConv(self.conv_first_nn)
            self.conv_gcn_first = tg.nn.GATConv(input_dim, hidden_dim)
        ...
        self.conv_gcn_hidden = nn.ModuleList([tg.nn.GATConv(hidden_dim, hidden_dim) for i in range(layer_num - 2)])
        self.conv_gcn_out = tg.nn.GATConv(hidden_dim, output_dim)
...
class P_A_SAGE(torch.nn.Module):
    def __init__(self, device, final_emb_size, input_dim, feature_dim, hidden_dim, output_dim,
                 feature_pre=True, layer_num=2, dropout=True, **kwargs):
        super(P_A_SAGE, self).__init__()
        self.feature_pre = feature_pre
        self.layer_num = layer_num 
        self.dropout = dropout
        ...
        if feature_pre:
            ...
            # self.conv_first_nn = nn.Linear(feature_dim, hidden_dim)
            # self.conv_gcn_first = tg.nn.GINConv(self.conv_first_nn)
            self.conv_gcn_first = tg.nn.SAGEConv(feature_dim, hidden_dim)
            ...
            # self.conv_first_nn = nn.Linear(input_dim, hidden_dim)
            # self.conv_gcn_first = tg.nn.GINConv(self.conv_first_nn)
            self.conv_gcn_first = tg.nn.SAGEConv(input_dim, hidden_dim)
        ...
        self.conv_gcn_hidden = nn.ModuleList([tg.nn.SAGEConv(hidden_dim, hidden_dim) for i in range(layer_num - 2)])
        self.conv_gcn_out = tg.nn.SAGEConv(hidden_dim, output_dim)
...
class P_CAT_GCN(torch.nn.Module):
    def __init__(self, device, final_emb_size, input_dim, feature_dim, hidden_dim, output_dim,
                 feature_pre=True, layer_num=2, dropout=True, **kwargs):
        super(P_CAT_GCN, self).__init__()
        self.feature_pre = feature_pre
        self.layer_num = layer_num 
        self.dropout = dropout
        ...
        self.deal_add = nn.Linear(hidden_dim * 3, hidden_dim)
        self.deal_add2 = nn.Linear(hidden_dim * 3, hidden_dim)
    ...
    def forward(self, data, dists):
        ...
        add = torch.cat((x_gcn, x_), dim=-1)
        x_gcn = self.deal_add(torch.cat((x_gcn, add), dim = -1))
        x_ = self.deal_add2(torch.cat((x_, add), dim = -1))
...
class P_SUM_GCN(torch.nn.Module):
    def __init__(self, device, final_emb_size, input_dim, feature_dim, hidden_dim, output_dim,
                 feature_pre=True, layer_num=2, dropout=True, **kwargs):
        super(P_SUM_GCN, self).__init__()
        self.feature_pre = feature_pre
        self.layer_num = layer_num 
        self.dropout = dropout
