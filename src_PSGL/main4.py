...
# for task in ['link', 'link_pair']:
    ...
    if args.dataset=='All':
        if task == 'link':
            ...
            # datasets_name =['communities','completeEmail', 'email', 'protein']
            # datasets_name = ['communities']
            # datasets_name = ['email']
    ...
    for dataset_name in datasets_name:
        ...
        for repeat in range(args.repeat_num):
            ...
            with open('epochs/{}_{}_{}.txt'.format(args.task, args.model, dataset_name), 'a') as f:
                f.write('repeat {}, dataset_name {}, loadtime {}\n'.format(repeat, dataset_name, time2 - time1))
            ...
            model = locals()[args.model](device = device, final_emb_size=final_emb, input_dim=input_dim, feature_dim=args.feature_dim,
            ...
            for epoch in range(args.epoch_num):
                ...
                for id, data in enumerate(data_list):
                    ...
                    # m = int(np.log2(data.x.shape[0]))
                    # args.anchor_num = m * m 
                    # anchorset_ids = preselect_anchor2(data, data_dists_list[id], layer_num=args.layer_num, anchor_num=args.anchor_num, device=device)
                ...
                if epoch % args.epoch_log == 0:
                    ...
                    for id, data in enumerate(data_list):
                        ...
                        if bestvalauc<auc_val:
                            ...
                            checkpoint = {'model': locals()[args.model](device = device, final_emb_size=final_emb, input_dim=input_dim, feature_dim=args.feature_dim,
                    ...
                    if epoch % 100 == 0:
                        ...
                        with open('epochs/{}_{}_{}.txt'.format(args.task, args.model, dataset_name), 'a') as f:
                            # tmp = model.state_dict()['pool.weight'].data.cpu().numpy()
                            # print(model.state_dict().keys())
                            tmp = model.state_dict()['pool.gnn.weight'].data.cpu().numpy()
                            for v in tmp:
                                f.write('{} '.format(v))
                            f.write('\n')

                    # print('pool.weight:\n', model.state_dict()['pool.weight'])
                    # print('Conv1.weight:\n', model.state_dict()['Conv1.weight'])
                    # print('pool.gnn.lin_l.weight', model.state_dict()['pool.gnn.lin_l.weight'])

                    if epoch % 100 == 0:
                        # with open('epochs/{}_{}_{}.txt'.format(args.task, args.model, dataset_name), 'a') as f:
                        #     f.write('repeat {}, epoch {}, Loss {:.4f}, Train AUC {:.4f}, Val AUC {:.4f}, Test AUC {:.4f}\n'.format(repeat, epoch, loss_train, auc_train, auc_val, auc_test))
                        with open('epochs/{}_{}_{}.txt'.format(args.task, args.model, dataset_name), 'a') as f:
                            f.write('repeat {}, epoch {}, Loss {:.4f}, Train AUC {:.4f}, Val loss {:.4f}, Val AUC {:.4f}, Test loss {:.4f}, Test AUC {:.4f}\n'.format(
                                repeat, epoch, loss_train,auc_train, loss_val, auc_val, loss_test, auc_test))
