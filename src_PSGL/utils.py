...
def graph_random_walks(i,edge_labels,args,graph,val_data,test_data):
    if args.edgelabel:
        if edge_labels[0] is not None:
            ...
            for val_index in range(val_data.shape[1]):
                edge_labels[i][val_data[0][val_index]][val_data[1][val_index]]=2
            for test_index in range(test_data.shape[1]):
                edge_labels[i][test_data[0][test_index]][test_data[1][test_index]]=2
...
def get_Random__single_anchor_node(n,dist,device,c=0.5):
    if args1.Num_Anchors=='logn':
        ...
        if n>=500:
            ...
            m=n//2
        copy = c
    elif args1.Num_Anchors == '16':
        m = 16
        copy = c
    elif int(args1.Num_Anchors)-int(args1.output_dim) == 0:
        m = int(args1.Num_Anchors)
        copy = c
...
def get_our_anchorset(n, num_anchors):
    anchorset_id = []
    for _ in range(num_anchors):
        anchorset_id.append(np.random.choice(n, size=1, replace=False))
    return anchorset_id

def get_our_dist(anchorset_id, dists, device):
...
# def get_our_dist(anchorset_id, dist, device):
...
#         # dist_max_temp, dist_argmax_temp = torch.max(dist_temp, dim=-1) 
#         dist_max_temp, dist_argmax_temp = max(dist_temp) 
#         dist_max[:,i] = dist_max_temp 
#         dist_argmax[:,i] = temp_id[dist_argmax_temp] 
#     return dist_max, dist_argmax 

def preselect_anchor2(data, data_dists, layer_num=1, anchor_num=32, anchor_size_num=4, device='cpu'):

    # anchorset_id = get_our_anchorset(data.num_nodes, anchor_num)
    anchorset_id = pooling_anchorset(data, anchor_num, device)
    # print(len(anchorset_id))
    # print(data.num_nodes)
    # pdb.set_trace()
    
    # pdb.set_trace()
    data.dists_max, data.dists_argmax, data.dists_max2 = get_our_dist(anchorset_id, data_dists, device)
    return anchorset_id


# num_features = 29
num_features = 1
Conv1 = GCNConv(num_features, args1.hidden_dim)
Pool_Model = TopKPooling(args1.hidden_dim)
device = torch.device('cuda:'+str(args1.cuda) if args1.gpu else 'cpu')
Pool_Model.to(device)
Conv1.to(device)

def pooling_anchorset(data, anchor_num, device):
    data.to(device)
    x, edge_index = data.x, data.edge_index
    # gcn = GCNConv(x.shape[1], 1).to(device)
    # print(x.shape)

    # score = gcn(x, edge_index).view(-1)
    x = Conv1(x, edge_index)
    # print(x.shape)
    x, _, _, _, perm, score = Pool_Model(x, edge_index, None, None)    
    # pdb.set_trace()
    # print(x.shape)

    # 输出模型参数
    # print('Pool_Model:\n')
    # for name, parms in Pool_Model.named_parameters():	
    #     print('-->name:', name)
    #     print('-->para:', parms)
    ...
    #     print("===")
    # print('Conv1:\n')
    # for name, parms in Conv1.named_parameters():	
    #     print('-->name:', name)
    #     print('-->para:', parms)
    #     print('-->grad_requirs:',parms.requires_grad)
    #     print('-->grad_value:',parms.grad)
    ...
    if len(perm) >= anchor_num:
        ...
        tmp = np.random.choice(perm.cpu(), anchor_num)
        for v in tmp:
            anchorset_id.append(np.array([v]))
